    ...
    <div>
      ...
      <transition name="fade" mode="out-in">
        ...
        <transition-group
          ...
          tag="div"
          v-else
          class="list-specialities"
          key="speciality-group"
        >
          <div
            class="list-specialities-item"
            @click="selectSpeciality(speciality)"
            v-for="(speciality, key) in filterSpecialities"
            :key="`speciality-item-${key}`"
          >
            <category
              :active="speciality.title==='Cardiology'"
              :img="speciality.img"
  ...
  computed: {
    ...
    filterSpecialities() {
      return this.specialitySearch.length === 0
        ? this.specialities
        : this.specialities.filter((category) =>
            category.title
              .toUpperCase()
              .includes(this.specialitySearch.toUpperCase())
          );
    },
    specialitiesCount() {
